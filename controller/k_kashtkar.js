...
const kKashtkarService = require('../services/k_kashtkar.service');
const { SUB_MODES, UNSUB_MODES, K_KASHTKAR } = require('../config/general.config.json');
...
async function getSubscriberDetail({ cellno }) {
    ...
    const result = await kKashtkarService.getSubscriberDetail(cellno);
...
async function updateSubscriber(payload) {
    ...
    const result = await kKashtkarService.updateSubscriber(payload);
...
async function subscribeUser(payload) {
    ...
    if (serviceIdOk.success) {
        ...
        serviceId = K_KASHTKAR.DEFAULT_SERVICE_ID;
    }

    const result = await kKashtkarService.subscribe(payload);
...
async function unsubscribeUser(payload) {
    ...
    const result = await kKashtkarService.unsubscribe(payload);
