...
async function updateUserProfile({ cellno, srvc_id, lang, name, occupation, province, district, tehsil, village, land, land_unit, alert_type, bc_mode, comment }) {
    if (!cellno) {
        return {
            responseCode: -100,
            message: "Params not found"
    ...
    const result = await kZamindarService.updateProfile(cellno, srvc_id, lang, name, occupation, province, district, tehsil, village, land, land_unit, alert_type, bc_mode, comment);
