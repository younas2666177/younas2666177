...
const zaraiDirectoryService = require('../services/zarai_directory.service');
const { SUB_MODES, UNSUB_MODES, ZARAI_DIRECTORY } = require('../config/general.config.json');
...
async function getSubscriberDetail({ cellno }) {
    ...
    const result = await zaraiDirectoryService.getSubscriberDetail(cellno);
...
async function updateSubscriber(payload) {
    ...
    const result = await zaraiDirectoryService.updateSubscriber(payload);
...
async function subscribeUser(payload) {
    ...
    const result = await zaraiDirectoryService.subscribe(payload);
...
async function unsubscribeUser(payload) {
    ...
    const result = await zaraiDirectoryService.unsubscribe(payload);
