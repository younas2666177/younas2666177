const {
    ...
    Sequelize,
    sequelize
} = require('../models/k_amdani');
const { K_AMDANI, DEFAULT_OPERATOR } = require('../config/general.config.json');
...
async function notSubSms(msisdn) {
    const content = "آپ کے نمبر پر یہ سروس موجود نہیں، سروس حاصل کرنے کے لیے sub لکھ کر اسی نمبر پر بھیجیں، یا 727240 ملا کر 1 کا بٹن دبائیں۔ شکریہ";
    await smsService.sendSms(msisdn, content, `${mask}`);
}
...
async function subscribe(payload) {
    ...
    try {
        //http://10.248.216.169:7081/charge?msisdn=msisdn&subMode=subMode&serviceId=serviceId&amount=amount&paidwall=paidwall&chargeInterval=
        ...
        if (foundServiceAmount.success && foundServiceAmount.data.amount) {
            ...
            chargeAmount = K_AMDANI.AMOUNT_TO_CHARGE;
            serviceId = K_AMDANI.DEFAULT_SERVICE_ID;
        }

        const graceExpireDateTime = format(addDays(new Date(), K_AMDANI.GRACE_INTERVAL_UNCHARGED), "yyyy-MM-dd HH:mm:ss");
        ...
        if (isReturningUser) {
            const chargingStatus = await SuccessChargeDetail.count({
                where: {
                    ...
                    responseCode: 100,
                    [Op.and]: [
                        sequelize.literal(`DATE_ADD(date(charged_dt), INTERVAL ${K_AMDANI.CHARGE_INTERVAL} DAY ) > date(now())`)
                    ]
                }
            });
            ...
            //smsService.sendSms(msisdn, "Khushaal Aamdani service hasil karnay ka shukria. Is service kay zariye khudana khaasta hospital dakhlay ki surat mei honay wali amdani ka nuksaan uthaye gay ab hum.Mazeed maloomat kay liye 727240 milayein. Shukria", 727240);
            ...
            //     axios.get(`http://localhost:3010/k-amdani-charging?cellno=${msisdn}`).catch(err => log("Error : ", err.message));
            // }
...
async function charging(msisdn) {
    ...
    const foundUserChargingStatus = await Subscriber.count({
        where: {
            msisdn: msisdn,
            subMode: {
                [Op.ne]: "API"
    ...
    const apiUrl = `/kz_charging?cellno=${msisdn}&token=${accessToken}&amount=${amount}&partnerID=${K_AMDANI.PARTNER_ID}&productID=${K_AMDANI.PRODUCT_ID}`

    const newChargeAttempt = await saveChargeAttempt(msisdn, serviceId, subMode, apiUrl);
    const chargingRequestResp = await tpService.sendChargingRequest(msisdn, amount, accessToken, K_AMDANI.PARTNER_ID, K_AMDANI.PRODUCT_ID);
    ...
    if (String(responseCode).trim() == "100" && IS_SUCCESS_CHARGE !== "N") {
        ...
        const nextChargeDT = addDays(new Date(), K_AMDANI.CHARGE_INTERVAL);
        let graceExpireDT = null;

        if (foundSubscriber.graceExpireDateTime) {
            const currentTime = new Date();
            const currentGraceTime = new Date(foundSubscriber.graceExpireDateTime);
            const extraGraceTime = addDays(currentGraceTime, K_AMDANI.GRACE_GRANT_INERVAL);

            if (currentTime <= extraGraceTime) {
                graceExpireDT = addDays(new Date(), K_AMDANI.GRACE_INTERVAL_RENEW_CHARGED);
            } else {
                graceExpireDT = addDays(new Date(), K_AMDANI.GRACE_INTERVAL_FIRST_CHARGED);
            }
...
async function sendInfoSms(msisdn, smsKeyword, amount, insuranceAmount, nextChargeDateTime) {
    ...
    smsService.sendSms(msisdn, smsText, 727240);
