const {
    ...
    Sequelize,
    sequelize
} = require('../models/k_kashtkar');
const axios = require('axios').default;
const { K_KASHTKAR, DEFAULT_OPERATOR } = require('../config/general.config.json');
...
async function notSubSms(msisdn) {
    const content = "آپ کے نمبر پر یہ سروس موجود نہیں، سروس حاصل کرنے کے لیے sub لکھ کر اسی نمبر پر بھیجیں، یا 727235 ملا کر 1 کا بٹن دبائیں۔ شکریہ";
    await smsService.sendSms(msisdn, content, `${mask}`);
}
...
async function subscribe(payload) {
    ...
    try {
        ...
        if (foundServiceAmount.success && foundServiceAmount.data.amount) {
            ...
            chargeAmount = K_kashtkar.AMOUNT_TO_CHARGE;
        }
...
async function sendInfoSms(msisdn, smsKeyword, amount, insuranceAmount, nextChargeDateTime) {
    ...
    await smsService.sendSms(msisdn, smsText, 727235);
