const {
    ...
    Sequelize,
    sequelize
} = require('../models/k_maweshi');
const axios = require('axios').default;
const { K_MAWESHI, DEFAULT_OPERATOR } = require('../config/general.config.json');
...
async function notSubSms(msisdn) {
    const content = "آپ کے نمبر پر یہ سروس موجود نہیں، سروس حاصل کرنے کے لیے sub لکھ کر اسی نمبر پر بھیجیں، یا 727244 ملا کر 1 کا بٹن دبائیں۔ شکریہ";
    await smsService.sendSms(msisdn, content, `${mask}`);
}
...
async function subscribe(payload) {
    ...
    try {
        ...
        if (foundServiceAmount.success && foundServiceAmount.data.amount) {
            ...
            chargeAmount = K_maweshi.AMOUNT_TO_CHARGE;
        }
...
async function charging(msisdn) {
    ...
    const apiUrl = `/kz_charging?cellno=${msisdn}&token=${accessToken}&amount=${amount}&partnerID=${K_MAWESHI.PARTNER_ID}&productID=${K_MAWESHI.PRODUCT_ID}`

    const newChargeAttempt = await saveChargeAttempt(msisdn, serviceId, subMode, apiUrl);
    const chargingRequestResp = await tpService.sendChargingRequest(msisdn, amount, accessToken, K_MAWESHI.PARTNER_ID, K_MAWESHI.PRODUCT_ID);
    ...
    if (String(responseCode).trim() == "100" && IS_SUCCESS_CHARGE !== "N") {
        ...
        const nextChargeDT = addDays(new Date(), K_MAWESHI.CHARGE_INTERVAL);
        let graceExpireDT = null;

        if (foundSubscriber.graceExpireDateTime) {
            const currentTime = new Date();
            const currentGraceTime = new Date(foundSubscriber.graceExpireDateTime);
            const extraGraceTime = addDays(currentGraceTime, K_MAWESHI.GRACE_GRANT_INERVAL);

            if (currentTime <= extraGraceTime) {
                graceExpireDT = addDays(new Date(), K_MAWESHI.GRACE_INTERVAL_RENEW_CHARGED);
            } else {
                graceExpireDT = addDays(new Date(), K_MAWESHI.GRACE_INTERVAL_FIRST_CHARGED);
            }
...
async function sendInfoSms(msisdn, smsKeyword, amount, insuranceAmount, nextChargeDateTime) {
    ...
    await smsService.sendSms(msisdn, smsText, 727244);
