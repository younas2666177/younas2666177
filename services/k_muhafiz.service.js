const {
    ...
    Sequelize,
    sequelize
} = require('../models/k_muhafiz');
const axios = require('axios').default;
const { K_MUHAFIZ, DEFAULT_OPERATOR } = require('../config/general.config.json');
...
async function notSubSms(msisdn) {
    const content = "آپ کے نمبر پر یہ سروس موجود نہیں، سروس حاصل کرنے کے لیے sub لکھ کر اسی نمبر پر بھیجیں، یا 727237 ملا کر 1 کا بٹن دبائیں۔ شکریہ";
    await smsService.sendSms(msisdn, content, `${mask}`);
}
...
async function subscribe(payload) {
    ...
    try {
        ...
        if (!foundServiceAmount.success) {
            chargeAmount = K_MUHAFIZ.AMOUNT_TO_CHARGE;
            serviceId = K_MUHAFIZ.DEFAULT_SERVICE_ID;
        } else {
            if (!foundServiceAmount.data.amount) {
                chargeAmount = K_MUHAFIZ.AMOUNT_TO_CHARGE;
                serviceId = K_MUHAFIZ.DEFAULT_SERVICE_ID;
            } else {
        ...
        const graceExpireDateTime = format(addDays(new Date(), K_MUHAFIZ.GRACE_INTERVAL_UNCHARGED), "yyyy-MM-dd HH:mm:ss");
        ...
        if (isReturningUser) {
            const chargingStatus = await SuccessChargeDetail.count({
                where: {
                    ...
                    responseCode: 100,
                    [Op.and]: [
                        sequelize.literal(`DATE_ADD(date(charged_dt), INTERVAL ${K_MUHAFIZ.CHARGE_INTERVAL} DAY ) > date(now())`)
                    ]
                }
            });
            ...
            await Subscriber.create({
                ...
                unsubDateTime: foundUnsubInfo.unsubDateTime,
                subMode: subMode,
                unsubMode: foundUnsubInfo.unsubMode,
                chargeAttemptDateTime: foundUnsubInfo.chargeAttemptDateTime,
                lastChargeDateTime: foundUnsubInfo.lastChargeDateTime,
                nextChargeDateTime: foundUnsubInfo.nextChargeDateTime,
                graceExpireDateTime: graceExpireDateTime,
                op: foundUnsubInfo.op,
                status: foundUnsubInfo.status,
                serviceId: serviceId
            });
            ...
            // if (compareDate(currentTime, foundUnsubInfo.nextChargeDateTime) || compareDate(foundUnsubInfo.graceExpireDateTime, currentTime)) {
            //     axios.get(`http://localhost:3010/k-muhafiz-charging?cellno=${msisdn}`).catch(err => log("Error : ", err.message));
            // }

            const foundSubInfo = await Subscriber.findOne({ where: { msisdn: msisdn } });
            console.log(foundSubInfo.serviceId);

            if (isReturningUser) {
                if (foundSubInfo.serviceId != foundUnsubInfo.serviceId) {
...
async function charging(msisdn) {
    ...
    const apiUrl = `/kz_charging?cellno=${msisdn}&token=${accessToken}&amount=${amount}&partnerID=${K_MUHAFIZ.PARTNER_ID}&productID=${K_MUHAFIZ.PRODUCT_ID}`

    const newChargeAttempt = await saveChargeAttempt(msisdn, serviceId, subMode, apiUrl);
    const chargingRequestResp = await tpService.sendChargingRequest(msisdn, amount, accessToken, K_MUHAFIZ.PARTNER_ID, K_MUHAFIZ.PRODUCT_ID);
    ...
    if (String(responseCode).trim() == "100" && IS_SUCCESS_CHARGE !== "N") {
        ...
        const nextChargeDT = addDays(new Date(), K_MUHAFIZ.CHARGE_INTERVAL);
        let graceExpireDT = null;

        if (foundSubscriber.graceExpireDateTime) {
            const currentTime = new Date();
            const currentGraceTime = new Date(foundSubscriber.graceExpireDateTime);
            const extraGraceTime = addDays(currentGraceTime, K_MUHAFIZ.GRACE_GRANT_INERVAL);

            if (currentTime <= extraGraceTime) {
                graceExpireDT = addDays(new Date(), K_MUHAFIZ.GRACE_INTERVAL_RENEW_CHARGED);
            } else {
                graceExpireDT = addDays(new Date(), K_MUHAFIZ.GRACE_INTERVAL_FIRST_CHARGED);
            }
...
async function sendInfoSms(msisdn, smsKeyword, amount, insuranceAmount, nextChargeDateTime) {
    ...
    await smsService.sendSms(msisdn, smsText, 727237);
