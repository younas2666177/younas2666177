const {
    ...
    Sequelize,
    sequelize
} = require('../models/k_zindagi');
const axios = require('axios').default;
const knex = require('knex');
const { K_ZINDAGI, DEFAULT_OPERATOR } = require('../config/general.config.json');
...
async function notSubSms(msisdn) {
    const content = "آپ کے نمبر پر یہ سروس موجود نہیں، سروس حاصل کرنے کے لیے sub لکھ کر اسی نمبر پر بھیجیں، یا 727234 ملا کر 1 کا بٹن دبائیں۔ شکریہ";
    await smsService.sendSms(msisdn, content, `${mask}`);
}
...
async function subscribe(payload) {
    ...
    try {
        ...
        if (!foundServiceAmount.success) {
            chargeAmount = K_ZINDAGI.AMOUNT_TO_CHARGE;
        ...
        const graceExpireDateTime = format(addDays(new Date(), K_ZINDAGI.GRACE_INTERVAL_UNCHARGED), "yyyy-MM-dd HH:mm:ss");
        ...
        if (isReturningUser) {
            const chargingStatus = await SuccessChargeDetail.count({
                where: {
                    ...
                    responseCode: 100,
                    [Op.and]: [
                        sequelize.literal(`DATE_ADD(date(charged_dt), INTERVAL ${K_ZINDAGI.CHARGE_INTERVAL} DAY ) > date(now())`)
                    ]
                }
            });
            ...
            //     axios.get(`http://localhost:3010/k-zindagi-charging?cellno=${msisdn}`).catch(err => log("Error : ", err.message));
            // }
...
async function charging(msisdn) {
    ...
    const apiUrl = `/kz_charging?cellno=${msisdn}&token=${accessToken}&amount=${amount}&partnerID=${K_ZINDAGI.PARTNER_ID}&productID=${K_ZINDAGI.PRODUCT_ID}`

    const newChargeAttempt = await saveChargeAttempt(msisdn, serviceId, subMode, apiUrl);
    const chargingRequestResp = await tpService.sendChargingRequest(msisdn, amount, accessToken, K_ZINDAGI.PARTNER_ID, K_ZINDAGI.PRODUCT_ID);
    ...
    if (String(responseCode).trim() == "100" && IS_SUCCESS_CHARGE !== "N") {
        ...
        const nextChargeDT = addDays(new Date(), K_ZINDAGI.CHARGE_INTERVAL);
        let graceExpireDT = null;

        if (foundSubscriber.graceExpireDateTime) {
            const currentTime = new Date();
            const currentGraceTime = new Date(foundSubscriber.graceExpireDateTime);
            const extraGraceTime = addDays(currentGraceTime, K_ZINDAGI.GRACE_GRANT_INERVAL);

            if (currentTime <= extraGraceTime) {
                graceExpireDT = addDays(new Date(), K_ZINDAGI.GRACE_INTERVAL_RENEW_CHARGED);
            } else {
                graceExpireDT = addDays(new Date(), K_ZINDAGI.GRACE_INTERVAL_FIRST_CHARGED);
            }
...
async function sendInfoSms(msisdn, smsKeyword, amount, insuranceAmount, nextChargeDateTime) {
    ...
    await smsService.sendSms(msisdn, smsText, 727234);
